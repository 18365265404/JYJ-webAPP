...
import '../assets/css/weHotelDetail.css';
...
export default class WeHotelDetail extends Component{
    ...
    constructor(props){
        ...
        this.state= {
            prevData:{
                ...
                activity:querystring.parse(props.location.search).activity,
                address:querystring.parse(props.location.search).address,
                describe:querystring.parse(props.location.search).describe,
                describe2:querystring.parse(props.location.search).describe2,
                describe3:querystring.parse(props.location.search).describe3,
                hotelType:querystring.parse(props.location.search).hotelType,
                price:querystring.parse(props.location.search).price,
                severalTimes:querystring.parse(props.location.search).severalTimes,
                starredHotel:querystring.parse(props.location.search).starredHotel,
                tableNumber:querystring.parse(props.location.search).tableNumber,
...
/*****************10/8 显示用户是否收藏***************/  
  ...
  render(){
    ...
    var length=this.state.prevData.starredHotel;
    var listStar=[];
...
/*****************10/8 min弹窗的内容***************/        
    ...
    return (
        <div className="weHotelDetail">
        
            <div style={{ height:'0rem'}}>

            </div>
            <div style={{ flex: '1',overflowY:"auto"}}>
            <div className="weHotelDetail-head" style={{backgroundImage: `url(${this.state.prevData.qiniu+this.state.prevData.imgUrl})`}}>
                <p className="public-back2" onClick={()=>{history.go(-1)}}>
                    <img src={"../assets/img/icon_back.png"} style={{width:"0.6rem",height:"0.4rem"}}/>
                </p>
                <div className="weHotelDetail-content">
                    <p>{this.state.prevData.name}<span>{this.state.prevData.price}元/桌</span></p>
                    <div className="star-line">
                        ...
                        <span>{this.state.prevData.hotelType} </span>
                    </div>
                  
                    <p className="weHotelDetail-content-p1">
                    {this.state.prevData.address}<Icon type="phone" theme="outlined" />
                    </p>

                    <div className="weHotelDetail-content-p2">
                        
                        <span>{this.state.prevData.activity}</span>
                    </div>
                   
                </div>
            </div>

            <div className="weHotelDetail-intr">
                <Tabs tabs={tabs}
                initialPage={0}
                swipeable={false}
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center',width: '9.25rem' ,overflow: 'hidden'}}>
                    <ul className="weHotelDetail-intr1">
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', }}>
                    <ul className="weHotelDetail-intr1">
                    {
                        this.state.menueList.map((item,index)=>{
                            return (
                                <li key={item.id}>
                                   <img src={this.state.prevData.qiniu+item.weddingGraphicMenu}/>
                                </li>
                            )
                        })
                        }
